package com.virtukch.dongiveupbe.domain.essential_product_status.service;

import com.virtukch.dongiveupbe.domain.essential_product_status.dto.EssentialProductStatusRequestDto;
...
import com.virtukch.dongiveupbe.domain.essential_product_status.dto.EssentialProductStatusResponseDto;
import com.virtukch.dongiveupbe.domain.essential_product_status.repository.EssentialProductStatusRepository;
...
public class EssentialProductStatusService {
    ...
    public EssentialProductStatus findById(Long essentialProductStatusId) {
        return essentialProductStatusRepository.findById(essentialProductStatusId)
                .orElseThrow(() -> new EntityNotFoundException("존재하지 않는 EssentialProductStatus ID입니다: " + essentialProductStatusId));
    }
    ...
    public EssentialProductStatusResponseDto updateProductStatusAmount(Long statusId, Long amount) {
        EssentialProductStatus productStatus = essentialProductStatusRepository.findById(statusId)
                .orElseThrow(() -> new EntityNotFoundException("해당 ID의 상품 상태를 찾을 수 없습니다. ID: " + statusId));
