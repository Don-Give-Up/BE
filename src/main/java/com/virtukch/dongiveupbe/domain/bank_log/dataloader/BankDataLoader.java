...
public class BankDataLoader implements CommandLineRunner {
    ...
    private void loadBankLogs() {
        if (bankLogRepository.count() == 0) {
            ...
            Long savingProductStatusId = 3L;  // savingProductStatusId를 3으로 고정

            // 생성자를 통해 BankLog 생성
            BankLog bankLog = new BankLog(gameMemberId, savingProductStatusId);

            bankLogRepository.save(bankLog);
            log.info("memberId 6에 대한 BankLog 스텁 데이터가 추가되었습니다. savingProductStatusId는 3으로 설정되었습니다.");
        } else {
